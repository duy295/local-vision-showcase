...
def candidate_existing_paths(p: str) -> List[str]:
    """Generate candidate filesystem paths for a normalized path.
    This mainly fixes JSON entries that point to .../train/Incremental class - Copy/<class>/img.jpg
    while the actual file lives at .../train/<class>/img.jpg.
    """
    cands = []
    ...
    lower = s.lower()
    replacements = [
        ('/train/incremental class - copy/', '/train/'),
        ('/incremental class - copy/', '/'),
        ('/incremental class/', '/'),
    ]
    for old, new in replacements:
        if old in lower:
            idx = lower.find(old)
            repl = s[:idx] + new + s[idx+len(old):]
            cands.append(repl)
...
def resolve_existing_path(p: str) -> Optional[str]:
    for cand in candidate_existing_paths(p):
...
def encode_missing_images(paths: List[str], emb_map: Dict[str, np.ndarray], model_name: str, device: str) -> Dict[str, np.ndarray]:
    ...
    for idx, p in enumerate(missing, 1):
        try:
            real_p = resolve_existing_path(p)
            if real_p is None:
                raise FileNotFoundError(p)
            img = Image.open(real_p).convert('RGB')
...
def plot_umap(
    ...
    for p in ref_paths:
        if p in emb_map:
            X.append(emb_map[p])
            labels.append('Ec Novel by Standard')

    for p in prop_paths:
        if p in emb_map:
            X.append(emb_map[p])
            labels.append('Ec Novel by Construction')

    for p in fewshot_paths:
        if p in emb_map:
            X.append(emb_map[p])
            labels.append('Few-sshot')
    ...
    # chỉ tăng kích thước figure
    plt.figure(figsize=(12, 10))

    plot_config = [
        ('Ec Novel by Standard', 'o', 220),
        ('Ec Novel by Construction', '^', 220),
        ('Few-shot', 's', 140),
    ]

    for name, marker, size in plot_config:
        idx = labels == name
        if idx.sum() > 0:
            plt.scatter(
                X_2d[idx, 0],
    ...
    plt.xlabel('Projected Semantic Dimension 1', fontsize=23)
    plt.ylabel('Projected Semantic Dimension 2', fontsize=23)
