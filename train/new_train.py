...
class FusionBackbone(nn.Module):
    """Shared front-end: per-modality encoders + cross-modal attention fusion.

    Used by both KANRanker and MLPRanker when use_feature_encoder=True, so the
    two model types are directly comparable (same input representation, only
    the head that follows the fused embedding differs).
    """
    def __init__(self, in_dim, hidden_dim=64, modality_embed_dim=32, attention_heads=4, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x, return_embeddings=False):
        ...
        if return_embeddings:
            emb["fused"] = fused
        h = self.fusion(fused)
        return (h, emb) if return_embeddings else h
...
class KANRanker(nn.Module):
    def __init__(self, in_dim, hidden_dim=64, grid_size=16, dropout=0.1,
        ...
        if use_feature_encoder:
            self.backbone = FusionBackbone(in_dim, hidden_dim, modality_embed_dim,
    ...
    def forward(self, x, return_embeddings=False):
        ...
        if self.use_feature_encoder:
            h, emb = self.backbone(x, return_embeddings=True)
        else:
            h = x
...
class MLPRanker(nn.Module):
    """Plain MLP alternative to KANRanker.

    Uses the same optional cross-modal FusionBackbone as KANRanker (so the two
    model types can be trained/compared apples-to-apples), but replaces the
    spline (KAN) layers with a standard Linear -> SiLU -> Dropout stack.
    """
    def __init__(self, in_dim, hidden_dim=64, dropout=0.1,
                 use_feature_encoder=True, modality_embed_dim=32, attention_heads=4,
                 grid_size=None):
        super().__init__()
        # grid_size accepted for CLI/checkpoint symmetry with KANRanker; unused here.
        self.use_feature_encoder = use_feature_encoder
        ...
        self.net = nn.Sequential(
            nn.Linear(mlp_in, hidden_dim), nn.SiLU(), nn.Dropout(dropout),
...
def build_model(model_type, in_dim, hidden_dim=64, grid_size=16, dropout=0.1,
                use_feature_encoder=True, modality_embed_dim=32, attention_heads=4):
    model_type = (model_type or "kan").lower()
    if model_type == "kan":
        return KANRanker(in_dim, hidden_dim, grid_size, dropout,
                         use_feature_encoder, modality_embed_dim, attention_heads)
    elif model_type == "mlp":
        return MLPRanker(in_dim, hidden_dim, dropout,
                         use_feature_encoder, modality_embed_dim, attention_heads)
    else:
        raise ValueError(f"Unknown model_type: {model_type!r} (expected 'kan' or 'mlp')")
...
def train_kan_ranker(groups, epochs, val_ratio, lr, weight_decay,
    ...
    model = build_model(model_type, in_dim, hidden_dim, grid_size, dropout,
                        use_feature_encoder).to(device)
...
def run_evaluation(model, data_list, X_mean, X_std, candidate_pool_k=20,
    ...
    for item in tqdm(data_list, desc="Eval"):
        ...
        if normalize_name(ec_t1) != normalize_name(true_raw) and is_correct:
            if flips < flip_log_limit:
                ...
                print(f"  EC final wrong: {ec_t1}  →  Model correct: {pred}  p={p_max:.3f}")
            flips += 1
...
def _load_checkpoint(path, device):
    ...
    cfg   = ckpt["config"]
    model_type = cfg.get("model_type", "kan")
    model = build_model(model_type, int(cfg["in_dim"]), int(cfg["hidden_dim"]),
                        int(cfg.get("grid_size", 16)), float(cfg["dropout"]),
                        bool(cfg.get("use_feature_encoder", True))).to(device)
...
if __name__ == "__main__":
    ...
    p.add_argument("--model_path",  type=str, default="weights/fusion_ranker.pth")
    p.add_argument("--output_json", type=str, default="output_json/fusion_eval.json")
    p.add_argument("--model_type",  type=str, default="kan", choices=["kan", "mlp"],
                   help="Which ranker head to train/eval: 'kan' (spline-based) or 'mlp' (plain MLP).")
